...
def train(nets, train_loader, train_batch_size, device, epoch, criteria, optimizers):
    for net in nets:
        net.train()
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(train_loader), desc=f'Training Epoch {epoch + 1}\t'):
        ...
        for i, (net, criterion, optimizer) in enumerate(zip(nets, criteria, optimizers)):
            optimizer.zero_grad()
            outputs = net(inputs)
...
def main(config):
    ...
    nets = utils.net_list(config['losses'], model, device)
    criteria = [utils.loss(loss) for loss in config['losses']]
...
if __name__ == '__main__':
    with open('config/config_loss.yaml', 'r') as f:
